...
export function ProjectContactForm(){
 ...
 const submit=async(data:ContactInput)=>{setStatus("idle");try{const response=await fetch("/api/contact",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(data)});const result=await response.json() as {message?:string};if(response.ok){setStatus("success");setMessage(result.message??"Solicitud enviada.");reset();}else{setStatus("error");setMessage(result.message??"No pudimos enviar la solicitud.");}}catch{setStatus("error");setMessage("No hay conexión. Inténtalo más tarde.");}};
 return <form onSubmit={handleSubmit(submit)} className="grid gap-5 sm:grid-cols-2" noValidate>
  ...
  <div className="flex items-end sm:justify-end"><button disabled={isSubmitting} className="min-h-12 w-full rounded-full bg-cyan-300 px-6 text-xs font-bold uppercase tracking-wider text-slate-950 hover:bg-white sm:w-auto">{isSubmitting?"Enviando…":"Enviar proyecto"}</button></div>
